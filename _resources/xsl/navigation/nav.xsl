...
<xsl:stylesheet 
									...
									<xsl:if test="boolean(ou:listAll($newPath1))">
										<ul>
											...
											<xsl:for-each select="ou:listDirs($newPath1)">
												...
												<li>
													asdfasdf<strong><xsl:apply-templates select="ou:fileLink($newRoot2, .)" /></strong>
	...
	<xsl:function name="ou:listDirs">
		...
		<xsl:for-each select="doc($docpath)/list/directory">
			<xsl:sort select="if($directory-order) then not(index-of($directory-order, .)) else ."/>
	...
	<xsl:function name="ou:listFiles">
		...
		<xsl:for-each select="doc($docpath)/list/file">
				<xsl:sort select="if($directory-order) then not(index-of($directory-order, ou:removeExt(.))) else ."/>
